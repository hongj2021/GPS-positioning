...
dtsv = af0+af1.*(rcvr_tow-toc)+af2.*(rcvr_tow-toc).^2+dtr; %%% satellite clock's correction
...
H = [ax ay az dtu1];
dx = (transpose(H)*H)^(-1)*transpose(H)*dpr0; % pseudoinverse of the parameters matrix
end
result_X = [Xu0(1) Yu0(1) Zu0(1)]; %%% receiver's position